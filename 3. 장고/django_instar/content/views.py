...
class Profile(APIView):
    def get(self, request):
        ...
        my_feed_list = Feed.objects.filter(email=email).order_by('-id')
        ...
        like_feed_list = Feed.objects.filter(id__in=like_list) .order_by('-id')                                                           # 또한, values_list() 안의 flat=True는 내가 가져온 데이터를 list의 형태로 가져오는 것이다.(flat=True를 한 다음에 전체를 list로 묶어줘야 리스트로 사용 가능.)
        ...
        feed_object_list = Feed.objects.filter(id__in=my_feed_list).order_by('-id')
        feed_list = []
        reply_list = []
        ...
        return render(request, 'content/profile.html', context=dict(my_feed_list=my_feed_list,
